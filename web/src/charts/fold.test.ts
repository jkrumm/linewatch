...
type Folded = { label: string; foldedFrom: number }

function source(labels: string[]): Source[] {
  return labels.map((label) => ({ label }))
}

describe('foldSourceIndex', () => {
  test('every source label resolves to the folded column that swallowed it', () => {
    const src = source(['a', 'b', 'c', 'd', 'e', 'f'])
    const folded: Folded[] = [
      { label: 'a', foldedFrom: 3 }, // consumes a, b, c
      { label: 'd', foldedFrom: 3 }, // consumes d, e, f
    ]
  ...
  test('a remainder group whose foldedFrom differs from its siblings still resolves every label', () => {
    ...
    const folded: Folded[] = [
      { label: 'a', foldedFrom: 2 },
      { label: 'c', foldedFrom: 2 },
      { label: 'e', foldedFrom: 1 },
    ]
    const index = foldSourceIndex(src, folded)
    expect(src.every((s) => index.has(s.label))).toBe(true)
    expect(index.get('a')).toBe(folded[0])
    expect(index.get('b')).toBe(folded[0])
   ...
   * resolve to nothing. A chart's `sourceIndex.get(key)` then returns `undefined` for those labels,
   ...
   */
  test('a fold that drops a remainder group leaves the trailing source labels unmapped', () => {
    const src = source(['a', 'b', 'c', 'd', 'e'])
    // foldedFrom sums to 3, not 5 — the last two source columns were never folded into anything.
    const folded: Folded[] = [{ label: 'a', foldedFrom: 3 }]
    const index = foldSourceIndex(src, folded)
